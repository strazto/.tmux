https://github.com/gpakosz/.tmux/blob/905122d13cc14ca07accabd3b87ef190fc4b61a8/.tmux.conf#L168-L1231
...
_apply_theme() {
  ...
  tmux_conf_theme_terminal_title=$(echo "$tmux_conf_theme_terminal_title" | sed \
    -e 's%#{circled_window_index}%#(cut -c3- ~/.tmux/apply.conf | sh -s _circled #I)%g' \
    -e 's%#{circled_session_name}%#(cut -c3- ~/.tmux/apply.conf | sh -s _circled #S)%g' \
    -e 's%#{username}%#(cut -c3- ~/.tmux/apply.conf | sh -s _username #{pane_tty} false #D)%g' \
    -e 's%#{hostname}%#(cut -c3- ~/.tmux/apply.conf | sh -s _hostname #{pane_tty} false #D)%g' \
    -e 's%#{username_ssh}%#(cut -c3- ~/.tmux/apply.conf | sh -s _username #{pane_tty} true #D)%g' \
    -e 's%#{hostname_ssh}%#(cut -c3- ~/.tmux/apply.conf | sh -s _hostname #{pane_tty} true #D)%g')
  ...
  tmux_conf_theme_status_left=$(echo "$tmux_conf_theme_status_left" | sed \
    -e "s%#{root}%#[fg=$tmux_conf_theme_root_fg]#[bg=$tmux_conf_theme_root_bg]#[$tmux_conf_theme_root_attr]#(cut -c3- ~/.tmux/apply.conf | sh -s _root #{pane_tty} #D)#[inherit]%g")
...
EOF
  ...
  case "$status_left $status_right" in
    *'#{battery_status}'*|*'#{battery_bar}'*|*'#{battery_hbar}'*|*'#{battery_vbar}'*|*'#{battery_percentage}'*)
      ...
      status_right="#(cut -c3- ~/.tmux/apply.conf | sh -s _battery)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{uptime_d}'*|*'#{uptime_h}'*|*'#{uptime_m}'*|*'#{uptime_s}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        ...
        -e 's/#\{(\?)?uptime_m/#\{\1@uptime_m/g' \
        -e 's/#\{(\?)?uptime_s/#\{\1@uptime_s/g')
      status_right="#(cut -c3- ~/.tmux/apply.conf | sh -s _uptime)$status_right"
      ;;
  esac
  ...
  case "$status_left $status_right" in
    *'#{loadavg}'*)
      ...
      status_right=$(echo "$status_right" | sed -E \
        -e 's/#\{(\?)?loadavg/#\{\1@loadavg/g')
      status_right="#(cut -c3- ~/.tmux/apply.conf | sh -s _loadavg)$status_right"
      ;;
  esac

  status_left=$(echo "$status_left" | sed 's%#{circled_session_name}%#(cut -c3- ~/.tmux/apply.conf | sh -s _circled #S)%g')
  status_right=$(echo "$status_right" | sed 's%#{circled_session_name}%#(cut -c3- ~/.tmux/apply.conf | sh -s _circled #S)%g')
